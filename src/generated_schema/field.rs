...
impl GeneratedStructFields{
    pub fn from(field: &RecordField,structure_name: &SanitizedName,default_namespace: &Option<String>)-> Result<Self> {
    ...
    pub fn write_struct_declaration_content(&self) -> Result<String>{
        ...
        {
            writeln!(content,"    #[serde(rename=\"{}\")]",self.name.original_name)?
        }
        if let Some(_) =self.default {
            writeln!(content,"    #[serde(default=\"{}::default_{}\")]",self.parent_struct_fullname,self.name.sanitized_name)?;
        }
        writeln!(content,"    pub {}: {},",self.name.sanitized_name,self.type_name)?;
    ...
    pub fn write_struct_default_method_content(&self) -> Result<Option<String>>{
        ...
        match &self.default {
            Some(default) => {
                let default_value_str= default.write_content()

                .map_err(|e|format!("{} => {e}",self.name.sanitized_name))?;
    
                Ok(format!("    #[inline(always)]\r\n    pub fn default_{}() -> {} {{ {} }}\r\n\r\n", 
                self.name.sanitized_name, self.type_name, default_value_str ).into())
            },
            None => Ok(None),
        }
    }
...
fn get_serde_with(field: &RecordField) -> Result<Option<&'static str>> {
    Ok(match field.schema{
        ...
        Schema::TimestampMillis => Some("#[serde(with = \"chrono::naive::serde::ts_milliseconds\")]"),
        Schema::TimestampMicros => Some("#[serde(with = \"chrono::naive::serde::ts_microseconds\")]"),
        Schema::LocalTimestampMillis => Some("#[serde(with = \"chrono::naive::serde::ts_milliseconds\")]"),
        Schema::LocalTimestampMicros => Some("#[serde(with = \"chrono::naive::serde::ts_microseconds\")]"),
        _ => None
    })
}