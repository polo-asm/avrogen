...
fn module_filename(parent_folder: PathBuf,namespace: &NamespaceInfo) -> PathBuf {
    ...
    return parent_folder.join(namespace.name.sanitized_name.to_string()).with_extension("rs");
}
...
pub fn write_to_structured_files(parent_folder: PathBuf,namespace: NamespaceInfo) -> Result<()>
{
    ...
    for (_, child) in namespace.children.into_iter() {
        ...
        let sub_folder: PathBuf= parent_folder.clone().join(namespace.name.sanitized_name.to_string());
...
fn create_current_file(namespace: &NamespaceInfo, file_path: PathBuf) -> Result<()> {
    ...
    debug!("Will create file: {}",file_path.clone().into_os_string().into_string().unwrap());
    ...
    for (_, content) in &namespace.generated_types {
